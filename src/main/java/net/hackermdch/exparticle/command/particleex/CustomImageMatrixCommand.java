...
import net.hackermdch.exparticle.network.CustomImageMatrixPayload;
...
public class CustomImageMatrixCommand {
    ...
    private static Command<CommandSourceStack> execute(boolean... flags) {
        return context -> {
            ...
            PacketDistributor.sendToPlayersInDimension(context.getSource().getLevel(),
                    new CustomImageMatrixPayload(effect, pos, path, scaling, matrixStr, dpb, attrExpression, speedExpression, speedStep, group));
            return 1;
        };
    }
}