...
public class VideoMatrixCommand {
    public static void register(LiteralArgumentBuilder<CommandSourceStack> parent, CommandBuildContext ctx) {
        parent.then(Commands.literal("video-matrix")
                .then(Commands.argument("name", ParticleArgument.particle(ctx)
                ).then(Commands.argument("pos", Vec3Argument.vec3()
                ).then(Commands.argument("path", FileArgumentType.file(VideoUtil.VIDEO_DIR)).executes(
                        (context) -> execute(
                                context,
    ...
    private static int execute(CommandContext<CommandSourceStack> context, ParticleOptions effect, Vec3 pos, String path, double scaling, String matrixStr, double dpb, double size, Vec3 speed, int age, String speedExpression, double speedStep, String group) {
        PacketDistributor.sendToPlayersInDimension(context.getSource().getLevel(), new VideoMatrixPayload(effect, pos, path, scaling, matrixStr, dpb, size, speed, age, speedExpression, speedStep, group));
        return 1;
    }
}
