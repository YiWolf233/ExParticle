...
public class Optimize {
    ...
    public static Expression optimizeTernary(Expression.TernaryExp exp) {
        if (exp.cond instanceof Expression.IntegerExp ie) return ie.val != 0 ? exp.trueExp : exp.falseExp;
        else if (exp.cond instanceof Expression.FloatExp fe) return fe.val != 0.0 ? exp.trueExp : exp.falseExp;
        if (exp.trueExp == exp.falseExp) return exp.trueExp;
        return exp;
    }
